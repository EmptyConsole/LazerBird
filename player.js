...
class player{   
  ...
  if(blocks[blocks.findIndex(hold => hold.bx === raySprite.x&&hold.by === raySprite.y&&hold.bsx === raySprite.width&&hold.bsy === raySprite.height) ?? 0]!=undefined){
      ...
      strokeWeight(5+random(-1,1));
      line(start.x,start.y,end.x,end.y);
      stroke("#DEC6B86D");
      strokeWeight(10+random(-1,1));
      line(start.x,start.y,end.x,end.y);
      strokeWeight(15+random(-1,1));
      line(start.x,start.y,end.x,end.y);
      noStroke();
      fill(255,50);
  ...
  player_move_and_slide(){
    ...
    for(var i=0; i<blocks.length; i++){
      ...
      if(
        ...
        (this.py+blocks[i].velocity.x+this.psy/2)<=(bl.preby-bl.bsy/2)&&
        (this.py+blocks[i].velocity.y+this.psy/2)>=(bl.by-bl.bsy/2)||
      ...
      //     blocks[w].refed = false;
      //   }
      //   blocks[newi[short]].refed = true;
        //trying to fix Hanson's glitch:
        if(bl.type==11){
          //Find the next velocity for the next frame and add that to the player
        }
        blocks[newi[short]].broken = true;
    }
    }
    }
    //Lower Y
