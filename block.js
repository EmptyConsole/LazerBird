...
class block{
  ...
  actions(){
    ...
    if(this.refed){
      player1.qxvel = this.velocity.x;
      //player1.qyvel = this.velocity.y;
      player1.py=this.by-this.bsy/2-player1.psy/2;
    }
    this.refed = false;
  }
  textDisplay(ind){
  ...
  form1(x,y,sx,sy,a,b,c,d){
    ...
    for(let i=0; i<sx/30; i++){
      for(let c=0; c<sy/30; c++){
       // ellipse(x-sx/2+i*30,y-sy/2+c*30,20,20);
     ...
     //   ellipse(plot.x,plot.y,20,20);
        ...
        for(let i2=0; i2<3; i2++){
          for(let c2= 0; c2<3; c2++){
            seter[i2][c2]=(blocks.findIndex(hold => rectHit(hold.bx,hold.by,plot.x-15+c2*30,plot.y-15+i2*30,hold.bsx,hold.bsy,20,20)&&(hold.type==this.type&&hold.life>0))>=0)+0;
          }
        }
  ...
  rotate(r*PI/180);
  imageMode(CORNER);
  image(tileSprite[0][4],-15,-15,15.1,15.1);
  image(tileSprite[0][5],-15,0,15.1,15.1);
  image(tileSprite[1][4],0,-15,15.1,15.1);
  image(tileSprite[1][5],0,0,15.1,15.1);
  pop();
   }
}
  check(x,y,z){
